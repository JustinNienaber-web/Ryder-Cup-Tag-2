...
function speicherschluessel() {
    return `rydercup-live-match-${aktivesMatch.match_number}`;
}

function matchschluessel(matchNummer) {
    return `rydercup-live-matchdaten-${matchNummer}`;
}
...
function scorecardAnzeigen() {
    ...
    hauptkarte.innerHTML = `
            ...
            <table class="scorecard">
                <thead>
                    <tr>
                        ...
                        <th>P1</th>
                        <th>P2</th>
                        <th class="summe-rot">RΣ</th>
                        <th class="summe-purple">PΣ</th>
                        <th>Pkt.</th>
                    </tr>
                </thead>
                <tbody>${zeilen}</tbody>
        ...
        <div class="scorecard-legende">
            <p><b>R1</b> ${text(personen[0].name)}</p>
            <p><b>R2</b> ${text(personen[1].name)}</p>
            <p><b>P1</b> ${text(personen[2].name)}</p>
            <p><b>P2</b> ${text(personen[3].name)}</p>
            <small>RΣ/PΣ = Netto-Aggregat · Punkt: R, P oder geteilt (–)</small>
        </div>
...
function liveAenderungVerarbeiten(payload) {
    const geaendert = payload.new;
    const index = liveMatches.findIndex(
        (match) => match.match_number === geaendert.match_number
    );

    if (index >= 0) liveMatches[index] = geaendert;
    else liveMatches.push(geaendert);
...
async function starten() {
    ...
    try {
        if (link.nummer) {
            aktivesMatch = await SUPABASE_ZUSTAND.ladeMatch(link.nummer);
            matchdatenLokalSpeichern(aktivesMatch);
            matchstart(aktivesMatch);
        } else {
            liveMatches = await SUPABASE_ZUSTAND.ladeAlleMatches();
            leaderboard(liveMatches);
            liveKanal = SUPABASE_ZUSTAND.beobachteMatches(liveAenderungVerarbeiten);
        }
    } catch (fehler) {
